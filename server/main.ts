...
const createCsvToDbTool = (env: Env) =>
  createTool({
    id: "CSV_TO_DB",
    description: "Fetch CSV data from a URL and insert it into a database table",
    inputSchema: z.object({
      csvUrl: z.string().url().describe("URL to the CSV file"),
      tableName: z.string().describe("Name of the database table to insert data into"),
      createTable: z.boolean().optional().default(false).describe("Whether to create the table if it doesn't exist"),
    }),
      ...
      try {
        ...
        if (context.createTable) {
          ...
          const sanitizedHeaders = headers.map(header => 
            header.replace(/[^a-zA-Z0-9_]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '')
          );
          ...
          const createTableSql = `CREATE TABLE IF NOT EXISTS "${context.tableName}" (${columnDefinitions})`;
          
          const createResult = await env.DECO_CHAT_WORKSPACE_API.DATABASES_RUN_SQL({
            sql: createTableSql,
            params: []
          });
        ...
        for (const row of dataRows) {
          ...
          const singleRowInsertSql = `INSERT INTO "${context.tableName}" ("${sanitizedHeaders.join('", "')}") VALUES (${placeholders})`;
          ...
          if (insertResult.result[0]?.success) {
            ...
            throw new Error(`Row insert failed: ${JSON.stringify(insertResult.result[0])}`);
          }
        }
        ...
        return {
          success: true,
          rowsInserted,
          message: `Successfully inserted ${rowsInserted} rows into table '${context.tableName}'`,
          tableCreated
        };
